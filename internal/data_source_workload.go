...
func (d *WorkloadDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	workloadState := WorkloadDataSourceModel{
		Data: WorkloadResults{
			...
			LastModified:   types.StringValue(workloadResponse.Data.GetLastModified().Format(time.RFC850)),
			CreatedAt:      types.StringValue(workloadResponse.Data.GetCreatedAt().Format(time.RFC3339)),
